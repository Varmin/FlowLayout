...
/**
 * 拖拽流布局:不应该使用普通的onclick侦听器，否则可能会有问题
 */
public class DragFlowLayout extends FlowLayout implements IViewManager {
     ...
     * set the adapter
     */
    public <T> void setDragAdapter(ItemAdapter<T> adapter) {
        if (adapter == null) {
  ...
  /*  @Override
    ...
    public class DragItemManager {
        ...
        public <T> List<T> getItems() {
            final ItemAdapter adapter = getDragAdapter();
        ...
        public void addItem(Object data) {
            final ItemAdapter mAdapter = getDragAdapter();
